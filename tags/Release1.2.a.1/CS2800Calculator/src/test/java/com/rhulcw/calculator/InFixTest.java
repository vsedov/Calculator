...
class InFixTest {
  ...
  void cand0Test()
    ...
    stringMap.put("2*20/2+(3+4)*3+2-6+15", (float) 52);
    stringMap.put("2*20/2+(3+4             )* 3 + 2 - 6 + 15", (float) 52);
    stringMap.put("5+(2*20)/2+(3+4)*3+2-6+15", (float) 57);
  ...
  /**
   * In fix Float Test.
   * Issues occouretd when infix was not evaluating floating point correcttly.
   *
   * @throws EmptyStackException the empty stack exception
   ...
   */
  @Test
  void inFixFloattEST() throws EmptyStackException, BadType, UserStringExecption {
      final Map<String, Float> stringMap = new HashMap<String, Float>();

      stringMap.put("(3.5 + 30)", (float) 33.5); //Inline Test 1
