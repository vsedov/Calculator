  ...
  public void userValue(ActionEvent event) throws UserStringExecption {
    ...
    try {
      if (Boolean.TRUE.equals(postFixOrInFix())) {
        output.setText(
                String.valueOf(InFixCalculator.evaluate(new StringEval().inFixCheck(userInput)))
        );
      } else {
        float x = new PostFixCaculator()
                .evaluate(new StringEval().postFixCheck(userInput));
